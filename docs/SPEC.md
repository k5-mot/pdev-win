...
Codex の user-level config を格納する。`setup.ps1` と同じディレクトリに `config/.codex` が存在する場合は、その内容を `$Root\.config\codex` へコピーする。LiteLLM など Codex の model provider 認証情報は launcher ではなく、この config 側で設定する。

### 3.8 `config/portable-tools.json`
...
15. `config/.codex` が存在する場合は、Codex user-level config を `$Root\.config\codex` へコピーする。
16. VS Code CLI で拡張機能を portable extensions dir にインストールする。
17. `$Root\VSCode.cmd`、`$Root\PowerShell.cmd` を生成する。
...
- `CODEX_HOME`、`CODEX_SQLITE_HOME` を設定する。Codex の model provider 認証情報は `$Root\.config\codex\config.toml` 側で扱う。
