...
try {
  ...
  pip install --no-warn-script-location setuptools wheel
#   pip install --no-cache-dir python-docx pypdf Pillow "docling-mcp[local]"
  pip install --no-cache-dir python-docx pypdf Pillow docling-mcp
  Write-Log "npm packages installing..." 'STEP'
  npm install -g npm gtop
  Write-Log "cargo packages installing..." 'STEP'
  cargo install ripgrep bat eza fd-find bottom du-dust zoxide
  ...
  Test-CommandPath 'rustc' 'rustc.exe' @('--version')
  Test-CommandPath 'cargo' 'cargo.exe' @('--version')
