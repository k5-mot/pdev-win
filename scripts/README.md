...
- [`Test-PinnedGitHubReleaseVersions.ps1`](Test-PinnedGitHubReleaseVersions.ps1)
  - `setup.ps1` と `setup_mise.ps1` に固定された GitHub Releases versions が latest release と一致しているか検証します。
- [`CheckDisk.ps1`](CheckDisk.ps1)
  - 指定したディレクトリ直下でサイズが大きいファイルまたはフォルダを Top 10 形式で特定します。
- [`Tree.ps1`](Tree.ps1)
  - ディレクトリツリーを表示する補助スクリプトです。
- [`Validate-DoclingChunkRemote.ps1`](Validate-DoclingChunkRemote.ps1)
  - docling-serve の `/v1/chunk/{path_name}/file/async` を使い、remote chunking が manifest と chunk files を生成できることを検証します。
