...
set "PATH=%SCOOP%\shims;%SCOOP%\apps\vscode\current\bin;%SCOOP%\apps\vscode\current;%SCOOP%\apps\python\current\Scripts;%SCOOP%\apps\python\current;%SCOOP%\apps\nodejs\current\bin;%SCOOP%\apps\nodejs\current;%npm_config_prefix%\bin;%PATH%"

for %%D in ("%HOME%" "%APPDATA%" "%LOCALAPPDATA%" "%TEMP%" "%PIP_CACHE_DIR%" "%npm_config_cache%" "%npm_config_prefix%" "%UV_CACHE_DIR%" "%SCOOP_STARTMENU_DIR%" "%INSTALL_ROOT%\.cache\powershell" "%INSTALL_ROOT%\.config\vscode\user-data" "%INSTALL_ROOT%\.config\vscode\extensions") do (
...
try {
    ...
    $script:LogPath = Join-Path $script:LogsRoot ('create-pdev-v2-{0}.log' -f (Get-Date -Format 'yyyyMMdd-HHmmss'))

    Write-Log -Level 'Info' -Message 'Create-Pdev_v2 started.'
    ...
    Write-Log -Level 'Success' -Message 'Create-Pdev_v2 finished successfully.'
    exit 0
} catch {
    if ($script:LogPath) {
        Write-Log -Level 'Error' -Message "Create-Pdev_v2 failed: $($_.Exception.Message)"
    } else {
        Write-Error $_
    }
    if ($userEnvBackup) {
